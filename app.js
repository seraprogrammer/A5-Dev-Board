const cards = [
  { label: "Shop Ease", title: "Fix Mobile Button Issue", description: "Fix the issue where the mobile menu button is not working properly." },
  { label: "Soft Pay", title: "Add Pay Success Modal", description: "Implement a modal that appears when a payment is successfully completed." },
  { label: "Meta", title: "Add new reaction 👋", description: "Introduce a new 'wave' reaction to the social feed." },
  { label: "Programming Hero", title: "Fix Video Loading Issue", description: "Resolve buffering issues on video lessons." },
  { label: "Google LLC", title: "Integrate AI search", description: "Enhance search functionality with AI-driven suggestions." },
];
...
// Render tasks
taskCards.innerHTML = cards.map((card, index) => `
...
document.querySelectorAll(".complete-btn").forEach((btn) => {
  btn.addEventListener("click", function () {
    ...
    if (taskCounter === 0) {
      setTimeout(() => alert("🎉 Congratulations! You have completed all your tasks."), 500);
    }
  });
});
